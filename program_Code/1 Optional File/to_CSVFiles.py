...
for audio_id, sentence in enumerate(sentences):
    ...
    temp_df = pd.DataFrame([{"filename":"audio-" + str(audio_id) + ".wav", "text":text}], columns=columns_List)
    df = df.append(temp_df)

# Exporting the csv file to the folder 
df.to_csv("CSV_Folder/csv_File-1.csv", index=False)
# df.to_csv("CSV_Folder/practise_csv1.csv", index=False)
